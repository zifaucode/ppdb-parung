...
class Panel_siswa extends CI_Controller {
	...
	public function prestasi()
	{
		...
		if(!isset($ceks)) {
			...
			$data['user']  			  = $this->db->get_where('tbl_siswa', "no_pendaftaran='$ceks'");
 			$data['judul_web'] 		= "Prestasi ".ucwords($data['user']->row()->nama_lengkap);

					$this->load->view('siswa/header', $data);
					$this->load->view('siswa/capaian_prestasi', $data);
					$this->load->view('siswa/footer');
		}
	}
	...
	public function cetak_daful() {
		$ceks = $this->session->userdata('no_pendaftaran');
		if(!isset($ceks)) {
			redirect('logcs');
		}
		$data['user'] 			= $this->db->get_where('tbl_siswa', "no_pendaftaran='$ceks'")->row();
		$data['judul_web'] 	= "Lembar Daftar Ulang ";
		...
		$this->load->view('siswa/daftar_ulang', $data);
	}


	public function cetak_keaslian() {
		$ceks = $this->session->userdata('no_pendaftaran');
		if(!isset($ceks)) {
			redirect('logcs');
		}
		$data['user'] 			= $this->db->get_where('tbl_siswa', "no_pendaftaran='$ceks'")->row();
		$data['judul_web'] 	= "Cetak Keaslian ".ucwords($data['user']->nama_lengkap);
		...
		$this->load->view('siswa/cetak_keaslian', $data);
	}

	public function cetak_deskripsi() {
		$ceks = $this->session->userdata('no_pendaftaran');
		if(!isset($ceks)) {
			redirect('logcs');
		}
		$data['user'] 			= $this->db->get_where('tbl_siswa', "no_pendaftaran='$ceks'")->row();
		$data['judul_web'] 	= "Cetak Deskripsi Capain ".ucwords($data['user']->nama_lengkap);
		...
		$this->load->view('siswa/cetak_deskripsi', $data);
	}

	public function cetak_borang() {
		$ceks = $this->session->userdata('no_pendaftaran');
		if(!isset($ceks)) {
			redirect('logcs');
		}
		$data['user'] 			= $this->db->get_where('tbl_siswa', "no_pendaftaran='$ceks'")->row();
		$data['judul_web'] 	= "Cetak Borang Isian Data  ".ucwords($data['user']->nama_lengkap);
		...
		$this->load->view('siswa/cetak_borang', $data);
	}
